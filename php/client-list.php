             	   ...
             	   $.each(  clientDetails , function(index , value){
            		  ...
            		  var details='';
             		  ...
             		  $.ajax({
                 	   ...
                 	   var client_details_clone = $('.client_details_clone').clone();
                 	   $( client_details_clone ).removeClass( 'display_none' );
                 	   $( client_details_clone ).removeClass( 'shop_detail_clone' );
                 	   $( client_details_clone ).find('.firstName').html( user_first_name );
                 	   $( client_details_clone ).find('.lastName').html( user_last_name );
                 	   $( client_details_clone ).find('.appoinment_date').html( appoinmentDate );
                 	   $( client_details_clone ).find('.sms_enable').addClass( 'sms_enable' + user_id );

                 	   if( ! jQuery.isEmptyObject( details )){
                 		  $( client_details_clone ).find('.appoinment_details').attr( 'data-content' , details );	   
                 	   }
                 	   else{
                 		  $( client_details_clone ).find('.appoinment_main').html('');
                 	   }

                 	  $( client_details_clone ).find('.book_new_appointment').attr( 'qrcode' , user_qr_code );
                 	  $( client_details_clone ).find('.edit_client').attr( 'qrcode' , user_qr_code );
                 	 $( client_details_clone ).find('.edit_client').attr( 'user_id' , user_id );
                 	$( client_details_clone ).find('.book_new_appointment').attr( 'user_id' , user_id );
                 	 
                 	   $('#client_details').append( client_details_clone.find('tr'));
             	   });
                },
                cache: false,
                contentType: false,
...
<table class="display_none client_details_clone">
	<tr>
    	<td class="firstName"></td>
        <td class="lastName"></td>
        <td class="appoinment_main"><span class="appoinment_date"></span><a class="appoinment_details" href="#" data-toggle="popover" title="Prossimi Apuntamenti" data-html="true" ><i class="fa fa-calendar" style="font-size:20px"></i></a></td>
        <td class="sms_option" >
        <input type="checkbox" data-on="on" data-off="off" data-toggle="toggle" data-onstyle="success" data-size="mini" data-style="ios">
        </td>
        <td ><a class="book_new_appointment" data-toggle="modal" href="#newappointment" style="color: blue;" >Book New Appoinment</a></td>
        <td><button class="btn btn-primary edit_client" type="button" ><i class="fa fa-pencil"></i></button></td>
    </tr>
</table>
